...
st_out:
        while (1)
        {
            ...
            case '"':
                state=ST_STR;
                goto st_str;
            default: /* ignore outside strings */
                break;
            }
        }
    case ST_STR:
st_str:
        ...
        {
ue:
            Error::sev(Error::EERROR) << "XPM: unexpected EOF" << (Error*)0;
        }
        else if (i=='"')
        {
            state=ST_OUT;
...
void XPMTok::read(char *buf, unsigned len)
{
    ...
    while (len--!=0)
    {
        ...
        // fprintf(stderr,"i=%d\n", i);
        if (i>=0 && i<=255) *buf++=i;
        else Error::sev(Error::EERROR) << "XPM: data expected" << (Error*)0;
    }
}
...
at_col:
    ...
    {
cexp:
        Error::sev(Error::EERROR) << "XPM: color expected" << (Error*)0;
    }
    return 0; /*notreached*/
}
            ...
            while (ht--!=0)
            {
                ...
                for (x=0; x<wd; x++)
                {
                    ...
                    if (0!=memcmp(p,pend,cpp))
                    {
                        p=tab;
                        lastcol=0;
                        while (p!=pend && 0!=memcmp(p,pend,cpp))
                        {
                            p+=cpp;
                            lastcol++;
                        }
