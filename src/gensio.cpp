...
void Encoder::vi_putcc(char c)
{
    vi_write(&c, 1);
}
int Decoder::vi_getcc()
{
    char ret;
    return vi_read(&ret, 1)==1 ? (unsigned char)ret : -1;
}
...
{
    FILE *tf;
    if (!unget.isEmpty() || (seekable_p && !isSeekable()))
    {
do_temp:
...
do_getc:
        ...
        if (f!=NULLP)
        {
            while ((i=MACRO_GETC(f))>=0 && i!=delimiter)
            {
                buf.vi_putcc(i);
                ftell_at++;
            }
...
lex:
    while (s[0]!='\0')   /* Interate throuh the template, substitute temporary filenames */
    {
        if (*s++=='%') switch (*s++)
            {
            case '\0':
            ...
            case '*': /* the optional unsafe string passed in param `i' */
                ...
                break;
            case 'd':
            case 'D': /* temporary file for encoded data output */
                pp=&tmpname;
put:
                ...
                break;
            case 'e':
            case 'E': /* temporary file for error messages */
                pp=&tmpename;
                goto put;
            case 's':
            case 'S': /* temporary source file */
    ...
    if (tmpname)
    {
        Files::tmpRemoveCleanup(tmpname());    /* already term0() */
        remove(tmpname());
    }
...
put:
    ...
    if (tmpsname)
    {
        Files::tmpRemoveCleanup(tmpsname());    /* already term0() */
        remove(tmpsname());
    }
    /* ^^^ Dat: remove() here introduces a race condition, but helps early error detection */
    /* </code similarity: Filter::PipeE::PipeE and Filter::PipeD::PipeD> */
