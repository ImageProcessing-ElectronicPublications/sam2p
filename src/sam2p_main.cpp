...
#include <stdlib.h>
void* __builtin_vec_new(unsigned len)
{
    return malloc(len);
}
...
void* __builtin_new(unsigned len)
{
    return malloc(len);
}
...
static unsigned sam2p_optval(char const* s, slen_t slen)
{
    if (slen==1)
    {
        ...
        {
        case 's':
            return OPT_SampleFormat;
        case 'l':
            return OPT_LoadHints;
        case 'h':
            return OPT_Hints;
        case 't':
            return OPT_TransferEncoding;
        case 'f':
            return OPT_TransferEncodingF;
        case 'c':
            return OPT_Compression;
...
#define APPEND_sf(val) do { if (sfx[val]==0) { sfx[val]=1; sft[sflen++]=val; } } while (0)
    Image::sf_t sft[Image::SF_max+1];
    char sfx[Image::SF_max+1];
    memset(sfx, 0, sizeof(sfx));
    unsigned sflen=0;
#define APPEND_co(val) do { if (cox[val]==0) { cox[val]=1; cot[colen++]=val; } } while (0)
    Rule::Cache::co_t cot[Rule::Cache::CO_max+1];
    char cox[Image::SF_max+1];
...
bad_label:
            ...
            if ((opt&15)==0)   /* no parameters */
            {
                if (*pend!='\0')
                {
                    badmsg="one_liner: don't give param to option: ";
                    goto bad_label;
                }
            ...
            else if ((opt&15)==2)     /* a mandatory parameter */
            {
                ...
                else if ((param=*++a)==(char const*)NULLP)
                {
                    badmsg="one_liner: missing param for option: ";
                    goto bad_label;
                }
            }
            else assert(0);
            ...
            case OPT_LoadHints:
                ...
                break;
            case OPT_TmpRemove:
                TmpRemove_p=GenBuffer::parseBool(param, paramlen);
                break;
            case OPT_Transparent:
                Transparent=param;
                break; /* Imp: is this good memory management */
            case OPT_Hints:
                Hints << '\n' << param;
                break;
            case OPT_PSL1:
                FileFormat=Rule::Cache::FF_PSL1;
                break;
            case OPT_PSLC:
                FileFormat=Rule::Cache::FF_PSLC;
                break;
            case OPT_PSL2:
                FileFormat=Rule::Cache::FF_PSL2;
                break;
            case OPT_PSL3:
                FileFormat=Rule::Cache::FF_PSL3;
                break;
            case OPT_DisplayJobFile:
                ...
                else if (0==GenBuffer::nocase_strcmp(param, "quiet"))
                {
                    buildProfile_quiet=true;
                    Error::setTopPrinted(Error::ERROR_CONT);
                } /* Dat: hide warnings, info etc. */ /* at Fri Aug 26 07:54:00 CEST 2005 */
...
all5:
                    ...
                    {
err_dimexp:
                        badmsg="one_liner: dimen expected: ";
                        goto bad_label;
                    }
                }
                else if (0!=(p2=one_pabbr(param,"horiz:"))
                         || 0!=(p2=one_pabbr(param,"x:")))
                {
                    if (MiniPS::Real::isDimen(p2)) LeftMargin=RightMargin=p2;
                    else goto err_dimexp;
                }
                else if (0!=(p2=one_pabbr(param,"vert:"))
                         || 0!=(p2=one_pabbr(param,"y:")))
                {
                    if (MiniPS::Real::isDimen(p2)) TopMargin=BottomMargin=LowerMargin=p2;
                    else goto err_dimexp;
                }
                else if (0!=(p2=one_pabbr(param,"left:")))
                {
                    if (MiniPS::Real::isDimen(p2)) LeftMargin=p2;
                    else goto err_dimexp;
                }
                else if (0!=(p2=one_pabbr(param,"right:")))
                {
                    if (MiniPS::Real::isDimen(p2)) RightMargin=p2;
                    else goto err_dimexp;
                }
                else if (0!=(p2=one_pabbr(param,"dpi:")))
                {
                    if (MiniPS::Real::isDimen(p2)) ImageDPI=p2;
                    else goto err_dimexp;
                }
                else if (0!=(p2=one_pabbr(param,"top:"))
                         || 0!=(p2=one_pabbr(param,"up:")))
                {
                    if (MiniPS::Real::isDimen(p2)) TopMargin=p2;
                    else goto err_dimexp;
                }
                else if (0!=(p2=one_pabbr(param,"bottom:"))
                         || 0!=(p2=one_pabbr(param,"down:")))
                {
                    if (MiniPS::Real::isDimen(p2)) BottomMargin=LowerMargin=p2;
                    else goto err_dimexp;
                }
                else if (0!=(p2=one_pabbr(param,"raise:")))
                {
                    if (MiniPS::Real::isDimen(p2)) LowerMargin=p2;
                    else goto err_dimexp;
                }
                else if (0!=(p2=one_pabbr(param,"lower:")))
                {
                    negLowerMargin=true;
                    if (MiniPS::Real::isDimen(p2)) LowerMargin=p2;
                    else goto err_dimexp;
            ...
            case OPT_TransferEncoding:
                ...
                {
inv_par:
                    badmsg="one_liner: invalid param for option: ";
                    goto bad_label;
                }
                break;
            ...
            case OPT_SampleFormat:
                ...
                while (*pend!='\0')
                {
                    ...
                    paramlen=pend-p;
                    if (4==paramlen && 0==memcmp(p,"stop",4))
                    {
                        do_stop_SampleFormat=true;
                        buildProfile_quiet=false;
                        break;
                    }
                    if (5==paramlen && 0==memcmp(p,"stopq",5))
                    {
                        do_stop_SampleFormat=true;
                        break;
                    }
            ...
            case OPT_Compression:
                ...
                else if (0==GenBuffer::nocase_strcmp(param, "dct"))
                {
                    APPEND_co(Rule::Cache::CO_DCT);
                    Hints << "\n/DCT<<>>";
                }
                else if (0==GenBuffer::nocase_strcmp(param, "jpg")
                         || 0==GenBuffer::nocase_strcmp(param, "jpeg"))
                {
                    APPEND_co(Rule::Cache::CO_JAI);
                    APPEND_co(Rule::Cache::CO_IJG);
                }
                else if (0==GenBuffer::nocase_strcmp(param, "ijg"))  APPEND_co(Rule::Cache::CO_IJG);
                else if (0==GenBuffer::nocase_strcmp(param, "g4"))
                {
                    APPEND_co(Rule::Cache::CO_Fax);
                    Hints << "\n/K -1";
                }
                ...
                else if (0==GenBuffer::nocase_strcmp(param, "g3:2d"))
                {
                    APPEND_co(Rule::Cache::CO_Fax);
                    Hints << "\n/K -2";
                }
        ...
        else     /* a selector or a normal argument */
        {
            ...
            if (pend-p>=2 && *pend==':')   /* an ImageMagick-style FileFormat selector */
            {
                ...
                else if (GenBuffer::nocase_strbegins(p, "PS2:"))
                {
                    FileFormat=Rule::Cache::FF_PSL2;
                    if (Scale==Rule::CacheHints::SC_default) Scale=Rule::CacheHints::SC_RotateOK;
                }
                else if (GenBuffer::nocase_strbegins(p, "PS:"))
                {
                    FileFormat=Rule::Cache::FF_eps;
                    if (Scale==Rule::CacheHints::SC_default) Scale=Rule::CacheHints::SC_RotateOK;
                }
                ...
                else if (GenBuffer::nocase_strbegins(p, "PNM:"))
                {
                    FileFormat=Rule::Cache::FF_PNM;
                ...
                else if (GenBuffer::nocase_strbegins(p, "PBM:"))
                {
                    FileFormat=Rule::Cache::FF_PNM;
                    APPEND_sf(Image::SF_Gray1);
                    do_stop_SampleFormat=true;
                }
                else if (GenBuffer::nocase_strbegins(p, "PGM:"))
                {
                    FileFormat=Rule::Cache::FF_PNM;
                    APPEND_sf(Image::SF_Gray8);
                    do_stop_SampleFormat=true;
                }
                else if (GenBuffer::nocase_strbegins(p, "PPM:"))
                {
                    FileFormat=Rule::Cache::FF_PNM;
                    APPEND_sf(Image::SF_Rgb8);
                    do_stop_SampleFormat=true;
                         ...
                         || GenBuffer::nocase_strbegins(p, "RLE:"))
                {
                    FileFormat=Rule::Cache::FF_BMP; /*APPEND_co(Rule::Cache::CO_RLE);*/
                }
...
unseekable:
                        ...
                        if (!f)
                        {
                            Error::sev(Error::ERROR_CONT) << "one_liner: cannot open" << " temporary file for `-' (stdin)" << (Error*)0;
                            badp=true;
                            continue;
                        }
                        tmpnam.term0();
                        ...
                        {
                            Error::sev(Error::ERROR_CONT) << "one_liner: cannot write" << " temporary file for `-' (stdin)" << (Error*)0;
                            badp=true;
                            continue;
                        }
                        // fprintf(stderr, "if=(%s)\n", InputFile);
                    ...
                    if (0!=fseek(stdin, 0L, 0))
                    {
                        if (c>=0) ungetc(c, stdin);    /* active test */
                        goto unseekable;
                    }
            ...
            else if (OutputFile==(char const*)NULLP)     /* Filename: STDOUT */
            {
                ...
                if (FileFormat==Rule::Cache::FF_default)   /* OutputFile; determine FileFormat from extension */
                {
                    ...
                    if (pend!=p)   /* have extension */
                    {
                        ...
                        else if (0==GenBuffer::nocase_strcmp(pend, "ps"))
                        {
                            FileFormat=Rule::Cache::FF_eps;
                            if (Scale==Rule::CacheHints::SC_default) Scale=Rule::CacheHints::SC_RotateOK;
                        }
                        ...
                        else if (0==GenBuffer::nocase_strcmp(pend, "pnm"))
                        {
                            FileFormat=Rule::Cache::FF_PNM;
                            APPEND_sf(Image::SF_Gray1);
                            APPEND_sf(Image::SF_Gray8);
                        ...
                        else if (0==GenBuffer::nocase_strcmp(pend, "pbm"))
                        {
                            FileFormat=Rule::Cache::FF_PNM;
                            APPEND_sf(Image::SF_Gray1);
                            do_stop_SampleFormat=true;
                        }
                        else if (0==GenBuffer::nocase_strcmp(pend, "pgm"))
                        {
                            FileFormat=Rule::Cache::FF_PNM;
                            APPEND_sf(Image::SF_Gray8);
                            do_stop_SampleFormat=true;
                        }
                        else if (0==GenBuffer::nocase_strcmp(pend, "ppm"))
                        {
                            FileFormat=Rule::Cache::FF_PNM;
                            APPEND_sf(Image::SF_Rgb8);
                            do_stop_SampleFormat=true;
                                 ...
                                 || 0==GenBuffer::nocase_strcmp(pend, "rle"))
                        {
                            FileFormat=Rule::Cache::FF_BMP; /*APPEND_co(Rule::Cache::CO_RLE);*/
                        }
    ...
    if (TransferEncoding==Rule::Cache::TE_default)   /* apply default if TransferEncoding is unspecified */
    {
        ...
        case Rule::Cache::FF_PSLC:
            TransferEncoding=(Rule::Cache::te_t)(cox[Rule::Cache::CO_ZIP]!=0 ||
                                                 cox[Rule::Cache::CO_LZW]!=0 ? 0+Rule::Cache::TE_A85 : 0+Rule::Cache::TE_Hex);
            break;
        /* ^^^ Dat: /Hex is default for /RLE */
        case Rule::Cache::FF_eps:
        case Rule::Cache::FF_PSL2:
        case Rule::Cache::FF_PSL3:
            TransferEncoding=Rule::Cache::TE_A85;
            break;
#if 0 /* useless bugfix */
        case Rule::Cache::FF_XPM: /* BUGFIX at Thu Jul 11 21:53:56 CEST 2002 */
            // assert(0);
            TransferEncoding=Rule::Cache::TE_ASCII;
            break;
#endif
        default:
            TransferEncoding=Rule::Cache::TE_Binary;
            break;
        }
    }
    ...
    if (jaip)
    {
        ...
        if (1!=jai_is_baseline_jpeg(ufd))
        {
            jaip=false;
            coc--;
            cox[Rule::Cache::CO_JAI]=0;
        }
        ufd->seek(0);
#if 0
    ...
    if (jaip)   /* Dat: might have changed to false */
    {
        ...
        LoadHints << ",jpeg-asis,";
        cot[0]=Rule::Cache::CO_JAI;
        colen=1; /* disable all other compression */
    }
...
int run_sam2p_engine(Files::FILEW &sout, Files::FILEW &serr, char const*const*argv1, bool helpp)
{
    ...
    else if (helpp || argv1[0]==(char const*)NULLP)
    {
help: /* help message */
...
int main(int, char const*const* argv)
{
    ...
    if (!quietp)
    {
        serr << "Available Loaders:";
        Image::printLoaders(serr);
        serr << ".\n";
    }
    init_applier();
    if (!quietp)
    {
        serr << "Available Appliers:";
        Rule::printAppliers(serr);
        serr << ".\n";
    }
