...
Rule::Applier::cons_t out_l23_work(GenBuffer::Writable& out, Rule::OutputRule*or_, Image::SampledInfo *sf)
{
    ...
    {
    case Rule::Cache::CO_None:
        break;
    case Rule::Cache::CO_ZIP:
        cp=PSEncoder::newFlateEncode(*tp, or_->cacheHints.Effort);
        break;
    case Rule::Cache::CO_LZW:
        cp=PSEncoder::newLZWEncode(*tp);
        break;
    case Rule::Cache::CO_RLE:
        cp=PSEncoder::newRunLengthEncode(*tp, or_->cacheHints.RecordSize);
        break;
    case Rule::Cache::CO_Fax:
        cp=PSEncoder::newCCITTFaxEncode(*tp, or_->cacheHints.K, or_->cacheHints.EncoderBPL, /*EndOfLine:*/ or_->cacheHints.K>0);
        break;
    ...
    case Rule::Cache::CO_IJG:
        cp=PSEncoder::newDCTIJGEncode(*tp, or_->cacheHints.EncoderColumns, or_->cacheHints.EncoderRows, or_->cacheHints.EncoderColors, or_->cacheHints.Quality);
        break;
    case Rule::Cache::CO_DCT:
    {
    ...
    if (cp!=tp) strcpy(closes," F closefile");
    else closes[0]='\0';
...
Rule::Applier::cons_t out_l1tr_work(GenBuffer::Writable& out, Rule::OutputRule*or_, Image::SampledInfo *sf)
{
    ...
    {
    case Image::SF_Transparent:
        t[2]='t';
        t[3]='r';
        break;
    case Image::SF_Opaque:
        t[2]='o';
        t[3]='p';
        break;
    case Image::SF_Bbox:
...
static void gen_tkey(char *tkey, GenBuffer::Writable& out, GenBuffer::Writable*&tp, GenBuffer::Writable*& cp, Rule::OutputRule*or_)
{
    Rule::Cache *cache=&or_->cache;
    tp=&out;
    if (cache->TransferEncoding==cache->TE_A85)
    {
        tkey[3]='8';
        tp=PSEncoder::newASCII85Encode (out, or_->cacheHints.TransferCPL);
    }
    else if (cache->TransferEncoding==cache->TE_Hex)
    {
        tkey[3]='h';
        tp=PSEncoder::newASCIIHexEncode(out, or_->cacheHints.TransferCPL);
    }
    else tkey[3]='b';
    cp=tp;
    if (cache->Compression==Rule::Cache::CO_RLE)
    {
        tkey[4]='r';
        cp=PSEncoder::newRunLengthEncode(*tp, or_->cacheHints.RecordSize);
    }
    else if (cache->Compression==Rule::Cache::CO_ZIP)
    {
        tkey[4]='z';
        cp=PSEncoder::newFlateEncode(*tp, or_->cacheHints.Effort);
    }
    else if (cache->Compression==Rule::Cache::CO_LZW)
    {
        tkey[4]='l';
        cp=PSEncoder::newLZWEncode(*tp);
    }
...
Rule::Applier::cons_t out_xpm_work(GenBuffer::Writable& out, Rule::OutputRule*or_, Image::SampledInfo *sf)
{
    ...
    {
        pend-=3;
        assert((iimg->getTransp())*3+p==pend);
        transp=true;
    }
    if (pal2ch)
    {
...
Rule::Applier::cons_t out_pnm_work(GenBuffer::Writable& out, Rule::OutputRule*or_, Image::SampledInfo *sf)
{
    ...
    if (alphaChannel!=NULLP)
    {
        ...
        else
        {
            ...
            alphaChannel->to8();
            p=alphaChannel->getRowbeg();
            len=alphaChannel->getRlen()*alphaChannel->getHt();
            while (len>=70)
            {
                smallen=70;
                t=tmp;
...
public:
    ...
    inline SimBuffer::B& getS()
    {
        return s;
    }
    inline SimBuffer::B const& getDir() const
    {
        return dir;
    }
...
void TIFFPrinter::aSHORT(SimBuffer::B &s, unsigned count, unsigned short const*val)
{
    if (le) while (count--!=0)
        {
            s << (char)(*val) << (char)(*val>>8);
            val++;
        }
    else while (count--!=0)
        {
            s << (char)(*val>>8) << (char)(*val);
            val++;
        }
}
void TIFFPrinter::aLONG (SimBuffer::B &s, unsigned count, slen_t const*val)
{
    ...
    if (le) while (count--!=0)
        {
            s << (char)(*val) << (char)(*val>>8) << (char)(*val>>16) << (char)(*val>>24);
            val++;
        }
    else while (count--!=0)
        {
            s << (char)(*val>>24) << (char)(*val>>16) << (char)(*val>>8) << (char)(*val);
            val++;
        }
}
...
Rule::Applier::cons_t out_tiffjai_work(GenBuffer::Writable& out, Rule::OutputRule*or_, Image::SampledInfo *sf)
{
    ...
    unsigned phot=0;
    bool inks=false, refe=false;
    switch (cs)
    {
    case Image::Sampled::CS_GRAYSCALE:
        phot=tp.PHOTOMETRIC_MINISBLACK;
        break;
    case Image::Sampled::CS_RGB:
        phot=tp.PHOTOMETRIC_RGB;
        break;
    case Image::Sampled::CS_YCbCr:
        phot=tp.PHOTOMETRIC_YCBCR;
        refe=true;
        break; /* preferred to RGB */
    case Image::Sampled::CS_CMYK:
        phot=tp.PHOTOMETRIC_SEPARATED;
        inks=true;
        break; /* preferred to RGB */
    default:
        Error::sev(Error::EERROR) << "TIFF6-JAI: color space " << (unsigned)cs << " not supported in TIFF-JPEG" << (Error*)0;
    }
...
Rule::Applier::cons_t out_tiff_work(GenBuffer::Writable& out, Rule::OutputRule*or_, Image::SampledInfo *sf)
{
    ...
    case Rule::Cache::CO_None:
        break;
    case Rule::Cache::CO_ZIP:
        compr=tp.COMPRESSION_DEFLATE;
        cp=PSEncoder::newFlateEncode(*cp, or_->cacheHints.Effort);
        break;
    case Rule::Cache::CO_LZW:
        compr=tp.COMPRESSION_LZW;
        cp=PSEncoder::newLZWEncode(*cp);
        break;
    /* vvv Dat: RunLengthEncode EOD (char 128) is OK and ignored in TIFF PackBits streams */
    case Rule::Cache::CO_RLE:
        compr=tp.COMPRESSION_PACKBITS;
        cp=PSEncoder::newRunLengthEncode(*cp, or_->cacheHints.RecordSize);
        break;
    case Rule::Cache::CO_Fax:
        ...
        {
            compr=tp.COMPRESSION_CCITTFAX3;
            or_->cacheHints.K=img->getHt();
        }
    ...
    if (compr==tp.COMPRESSION_JPEG) switch (jp->getColorSpace())
        ...
        default:
            Error::sev(Error::EERROR) << "TIFF6: color space " << (unsigned)jp->getColorSpace() << " not supported in TIFF-JPEG" << (Error*)0;
        }
...
Rule::Applier::cons_t out_png_work(GenBuffer::Writable& out, Rule::OutputRule*or_, Image::SampledInfo *sf)
{
    ...
    *p++=crc; /* 29: IHDR CRC */
    ...
    if (colortype==3)
    {
        ...
        *p++=0;
        *p++=ncols3>>8;
        *p++=ncols3; /* 0: PLTE chunk length */
        *p++='P';
        *p++='L';
        *p++='T';
        *p++='E'; /* 4: PLTE chunk name */
        out.vi_write(tmp, 8);
        if (transp)
        {
            PTS_dynamic_cast(Image::Indexed*,img)->makeTranspZero();
            p=img->getHeadp();
            p[0]=p[1]=p[2]=(char)0; /* force black backround (for consistency) */
        }
    ...
    *p++=rlenht>>16;
    *p++=rlenht>>8;
    *p++=rlenht; /* 0: IDAT chunk length */
    *p++='I';
    *p++='D';
    *p++='A';
    *p++='T'; /* 4: IDAT chunk name */
    out.vi_write(tmp, 8);
...
Rule::Applier::cons_t out_bmp_work(GenBuffer::Writable& out, Rule::OutputRule*or_, Image::SampledInfo *sf)
{
    ...
    if (or_->cache.Compression==or_->cache.CO_RLE)
    {
        ...
        while (htc--!=0)   /* BMP stores rows from down */
        {
            ...
            pend-=rlen;
            *crow2++='\0';
            *crow2++='\0'; /* signal end of compressed data row */
    ...
    *p++='M'; /* magic number header */
    ...
    out.vi_write(bmphead, p-bmphead);
    delete [] bmphead;
#if 0
