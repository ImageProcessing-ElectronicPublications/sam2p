...
public:
    ...
    virtual ~FlateEncode()
    {
        zlib_deflateEnd(&zs);    /* Dat: ==Z_OK check (for unflushed buffers) omitted */
    }
    virtual void vi_write(char const*buf, slen_t len);
protected:
...
{
    register s_t sold;
    register char const*p=buf;
    char const*pend=p+len;
    (void)sold;
    ...
    */
    ...
    while ((abufleft=ABUFSIZE-abuflen)<len)
    {
        ...
        abuf[1]=(char)ABUFSIZE;
        abuf[2]=(char)(ABUFSIZE>>8);
        abuf[3]=(char)~(ABUFSIZE);
        abuf[4]=(char)~(ABUFSIZE>>8);
        if (!had_header)
        {
            out.vi_write("\x78\x01", 2);
            had_header=true;
...
void ASCII85Encode::vi_write(char const*buf, slen_t len)
{
    if (len==0)
    {
        ...
        if (ascii85left!=4)
        {
            ...
            buf_ -= q * ((PTS_UINT32_T)85*85*85*85);
            q = buf_ / ((PTS_UINT32_T)85*85*85);
            encoded[1] = q + '!';

            buf_ -= q * ((PTS_UINT32_T)85*85*85);
            q = buf_ / (85*85);
            encoded[2] = q + '!';
    ...
    else
    {
        ...
        do
        {
            while (len!=0 && aleft!=0)
            {
                abuf=(abuf<<8)+*(unsigned char const*)buf++;
                len--;
                aleft--;
            }
            if (aleft!=0) break;
...
yes:
    ...
    if (saved_rep!=0)   /* Try to increase the repeating sequence */
    {
        ...
        assert(len>=1);
        b=(saved+1)[0];
        umax=len>128?128:len;
        if (umax>record_left) umax=record_left;
        /* fprintf(stderr,"um1=%u\n", umax); */
        if (umax>128-saved_rep) umax=128-saved_rep;
        assert(umax>=1);
        j=0;
        while (j!=umax && buf[j]==b)
        {
            j++;
            saved_rep++;
        }
...
put_rep: /* Found a maximal repeat width */
    ...
    else     /* Try to increase the non-repeating sequence */
    {
        ...
        if (buf[0]==(saved+1)[saved_c-1])   /* this decision might be sub-optimal */
        {
            ...
            saved_c=1;
            saved_rep=2; /* make the new block a >=2 repeat */
            record_left--;
            buf++;
            if (0==--len) return;
            goto again;
        ...
        assert(umax>=1);
        j=1;
        while (j!=umax && buf[j]!=buf[j-1]) (saved+1)[saved_c++]=buf[j++];
...
void FlateEncode::vi_write(char const*buf, slen_t len)
{
    ...
    */
    ...
    while (len>=0x8000)
    {
        fs->deflate2(const_cast<char*>(buf),0x8000, fs);
        len-=0x8000;
        buf+=0x8000;
    }
...
void Lzw_codecEncode::vi_write(char const*buf, slen_t len)
{
    p.vi_write(buf,len);
}
...
void CjpegEncode::vi_write(char const*buf, slen_t len)
{
    p.vi_write(buf,len);
}
...
void CjpegEncode::P::vi_copy(FILE *f)
{
    ...
    {
bad:
        Error::sev(Error::EERROR) << "CjpegEncode: cjpeg created bad JPEG" << (Error*)0;
    }
...
void GSEncode::vi_write(char const*buf, slen_t len)
{
    p.vi_write(buf,len);
}
...
#else
    SimBuffer::B fp("<</Effort ");
    (fp << Effort << ">>/FlateEncode").term0();
    PSEncoder *ret=new GSEncode(out_, fp());
#endif
    // ret->shortname="Fla";  ret->longname="Flate";
    // ret->filter_psname << fp;
...
{
    SimBuffer::B fp;
    (fp << RecordSize << "/RunLengthEncode").term0();
                                  ...
                                  )
{
    ...
    {
        tmp.clear();
        tmp.vi_write((char const*)HSamples, (unsigned)Colors);
        fp << "/HSamples ";
        fp.appendDumpPS(tmp,true);
    }
    if (VSamples!=(unsigned char const*)NULLP)
    {
        tmp.clear();
        tmp.vi_write((char const*)VSamples, (unsigned)Colors);
        fp << "/VSamples ";
        fp.appendDumpPS(tmp,true);
    }
...
void PNGPredictorNone::vi_write(char const*buf, slen_t len)
{
    ...
    if (opleft==0)
    {
        opleft=rlen;    /* Scanline (row) header: describes predictor used */
        out.vi_write("\0",1);
    }
    while (len>opleft)
    {
        ...
        len-=opleft;
        opleft=rlen;
        out.vi_write("\0",1); /* Scanline (row) header: describes predictor used */
    }
...
void PNGPredictorSub::vi_write(char const*buf, slen_t len)
{
    ...
    while (p!=pend0)
    {
        ...
        *op++=((i-((h>>bpccpp)))/*&255*/);
        ...
        {
            h=0;
            out.vi_write((char*)obuf-1,rlen+1);
            op=obuf;
            opleft=rlen;
        }
    }
...
static inline unsigned abs_(unsigned i)
{
...
void PNGPredictorPaeth::vi_write(char const*buf, slen_t len)
{
    ...
    while (p!=pend0)
    {
        ...
        i=*p;
        *op++=i-paeth_predictor((h>>bpccpp)&255, *oq, (g>>bpccpp)&255);
        h=(h<<8)|i;
        g=(g<<8)|*oq;
        *oq++=*p++;
        // i=*p; *op++=i-h; h=i; *oq++=*p++;
    ...
    while (p!=pend0)
    {
        ...
        *(o_prior-opleft)=*p++;
        if (--opleft==0)
        {
            ...
            min_weight=0;
            beg=(end=(signed char*)o_0)-rlen;
            while (beg!=end) min_weight+=*beg++;
            if (min_weight<0) min_weight*=-1;
            for (cur_pred=1; cur_pred<=4; cur_pred++)
            {
                cur_weight=0;
                beg=(end=(signed char*)oo[cur_pred])-rlen;
                while (beg!=end) cur_weight+=*beg++;
    ...
    while (p!=pend0)
    {
        ...
        *(o_prior-opleft)=*p++;
        if (--opleft==0)
        {
            ...
            min_weight=0;
            beg=(end=(unsigned char*)o_0)-rlen;
            while (beg!=end) min_weight+=*beg++;
            for (cur_pred=1; cur_pred<=4; cur_pred++)
            {
                cur_weight=0;
                beg=(end=(unsigned char*)oo[cur_pred])-rlen;
                while (beg!=end) cur_weight+=*beg++;
                if (cur_weight<min_weight)
                {
                    min_weight=cur_weight;
    ...
    while (p!=pend0)
    {
        ...
        *(o_prior-opleft)=*p++;
        if (--opleft==0)
        {
            ...
            min_weight=0;
            beg=(end=(signed char*)o_0)-rlen;
            while (beg!=end) min_weight+=abs_(*beg++);
            for (cur_pred=1; cur_pred<=4; cur_pred++)
            {
                cur_weight=0;
                beg=(end=(signed char*)oo[cur_pred])-rlen;
                while (beg!=end) cur_weight+=abs_(*beg++);
                if (cur_weight<min_weight)
                {
                    min_weight=cur_weight;
                    min_pred=cur_pred;
...
Encoder* PSEncoder::newPredictor(GenBuffer::Writable &out_, unsigned char type, unsigned char bpc_, slen_t columns_, unsigned char cpp_)
{
    ...
    case 1:
        return new Filter::VerbatimE(out_);
    case 2:
        return new TIFFPredictor2(out_, bpc_, columns_, cpp_);
    case 10:
        return new PNGPredictorNone(out_, bpc_, columns_, cpp_);
    case 11:
        return new PNGPredictorSub(out_, bpc_, columns_, cpp_);
    case 12:
        return new PNGPredictorUp(out_, bpc_, columns_, cpp_);
    case 13:
        return new PNGPredictorAverage(out_, bpc_, columns_, cpp_);
    case 14:
        return new PNGPredictorPaeth(out_, bpc_, columns_, cpp_);
    case 15:
        return new PNGPredictorAuto(out_, bpc_, columns_, cpp_);
    case 45:
        return new PNGPredictorAutoBadUnsigned(out_, bpc_, columns_, cpp_); /* pts' extension */
    case 55:
        return new PNGPredictorAutoBadSigned(out_, bpc_, columns_, cpp_); /* pts' extension */
    }
