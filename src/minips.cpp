...
eof:
    ...
    case '<':
        ...
        {
uf_hex:
            Error::sev(Error::EERROR) << "miniPS: unfinished hexstr" << (Error*)0;
        }
        if (c=='<') return '<';
...
redo:
            ...
            case 'b':
                b << '\010';
                break; /* \b and \a conflict between -ansi and -traditional */
            case 'f':
                b << '\f';
                break;
            default:
...
uf_str:
        Error::sev(Error::EERROR) << "miniPS: unfinished str" << (Error*)0;
        break;
    case '/':
...
MiniPS::VALUE MiniPS::Dict::get(char const*keys, slen_t keylen)
{
    ...
    char *has=((Mapping::H*)ptr)->get(keys,keylen);
    VALUE ret=Qundef;
    if (has!=NULLP) memcpy(&ret, has, sizeof(VALUE));
    return ret;
}
...
{
    char *beg=0;
    slen_t len=0; /* pacify g++-2.91 */
    Real::metric_t metric;
    Real *r=0; /* pacify g++-2.91 */
    VALUE v, w;
    if (master!=NULLP)
    {
from_master:
    ...
    case '(':
    {
        beg=tok->lastTokVal().bb->begin_();
        len=tok->lastTokVal().bb->getLength();
        VALUE v=(VALUE)new String(beg, len);
        i=tok->yylex();
        beg=tok->lastTokVal().bb->begin_();
        len=tok->lastTokVal().bb->getLength();
        if (i!='E' || len!=3 || 0!=memcmp(beg,"run",3))
        {
            unread=i;
            return v;
        }
    ...
    case '[':
    {
        Array *ap=new Array();
        VALUE v;
        while (Qundef!=(v=parse1(']', sev)))
        {
            if (v==Qerror) return Qerror;
            ap->push(v);
        }
        return (VALUE)ap;
    }
...
bool MiniPS::isEq(MiniPS::VALUE v, double d)
{
    ...
    case T_INTEGER:
        ...
        break;
    default:
        Error::sev(Error::EERROR) << "isEq: number expected" << (Error*)0;
    }
    if (dif<0.0) dif=-dif;
...
void MiniPS::dumpScale(GenBuffer::Writable &out, VALUE v)
{
    ...
    case T_INTEGER:
        ...
        break;
    default:
        Error::sev(Error::EERROR) << "dumpScale: number expected" << (Error*)0;
    }
    if (d == -72.0) d = 72.0;
    ii = (ii_t)d / 72 * 72;
