...
added:
        ...
        memcpy(p->keydata+=datalen, key, p->keylen=keylen);
        len++;
    ...
    if (p->keylen==keylen && 0==memcmp(p->keydata, key, keylen))
    {
updated:
...
void Mapping::DoubleHash::rehash()
{
    ...
    // assert( minlen <= len);
    if (scale==old_scale && used<=maxused)
    {
        assert(obj_assert());
        return;
    }
...
again:
    ...
    {
        // printf("xscale=%u\n", scale);
        if (0==(alloced=(d15_primes+2)[scale]))
        {
            assert(0);    /* Imp: better overflow reporting */
            abort();
        }
        if (alloced<=4000U) maxused=(alloced*15)>>4;
