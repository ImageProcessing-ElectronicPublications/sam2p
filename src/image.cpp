...
unsigned char Image::Sampled::minRGBBpc() const
{
    ...
    for (htc=0; htc<ht; htc++)
    {
        ...
        for (p=crow; p!=pend; p++)
        {
            if ((*p&15)*17!=*p)
            {
                delete [] crow;    /* 4 bits are not enough */
                return 8;
            }
...
void Image::Indexed::to8()
{
    to8nomul();
}
Image::Indexed* Image::Indexed::toIndexed()/* const*/
{
    return this;
}
Image::RGB*     Image::Indexed::toRGB(unsigned char bpc_)/* const*/
{
    return toRGB0(bpc_);
}
Image::Gray*    Image::Indexed::toGray(unsigned char bpc_)/* const*/
{
    return toGray0(bpc_);
}
...
void Image::Indexed::packPal()
{
    ...
    if (newTransp!=-1 && newTransp!=(int)newNcols-1)
    {
        ...
        memset(newpal+3*newLast, 0, 3);
        transpc=0; /* make it black */
...
Image::RGB    * Image::Gray::toRGB(unsigned char bpc_)/* const*/
{
    return toRGB0(bpc_);
}
Image::Gray*    Image::Gray::toGray(unsigned char bpc_)/* const*/
{
    return bpc==bpc_ ? this : toGray0(bpc_);
}
...
Image::Gray*    Image::RGB::toGray(unsigned char bpc_)/* const*/
{
    return toGray0(bpc_);
}
Image::RGB*     Image::RGB::toRGB(unsigned char bpc_)/* const*/
{
    return bpc==bpc_ ? this : toRGB0(bpc_);
}
Image::Indexed* Image::RGB::toIndexed()/* const*/
{
    return toIndexed0();
}
bool Image::RGB::canGray() const
{
    return false;
}
...
Rule::Sampled *Rule::load(char const* filename)
{
    ...
    while (p!=NULLP)
    {
        if (NULLP!=(reader=p->checker(buf,buf+Applier::MAGIC_LEN)))
        {
            return reader(f);
        }
        p=p->next;
...
unsigned Image::printLoaders(GenBuffer::Writable &out)
{
    ...
    while (p!=NULLP)
    {
        if (p->checker!=(Loader::checker_t)0 && p->format!=(char const*)NULLP)
        {
            num++;
            out << ' ' << p->format;
        }
...
Image::SampledInfo::~SampledInfo()
{
    ...
    if (imgs!=NULLP)
    {
        Image::Indexed::delete_separated(imgs);
        delete [] imgs;
    }
}
...
bool Image::SampledInfo::setSampleFormat(sf_t sf_, bool WarningOK, bool TryOnly, Sampled::rgb_t Transparent)
{
    ...
    case SF_Indexed1:
        ...
        return true;
    case SF_Mask:
        if (!hasTransp && nncols==2 && PTS_dynamic_cast(Indexed*,img)->wouldSetTranspc(Transparent))
        {
            hasTransp=true;
            --nncols;
        }
    ...
    case SF_Transparent2:
        if (!hasTransp && nncols==4 && PTS_dynamic_cast(Indexed*,img)->wouldSetTranspc(Transparent))
        {
            hasTransp=true;
            --nncols;
        }
    ...
    case SF_Transparent4:
        if (!hasTransp && nncols==16 && PTS_dynamic_cast(Indexed*,img)->wouldSetTranspc(Transparent))
        {
            hasTransp=true;
            --nncols;
        }
    ...
    case SF_Transparent8:
        if (!hasTransp && nncols==256 && PTS_dynamic_cast(Indexed*,img)->wouldSetTranspc(Transparent))
        {
            hasTransp=true;
            --nncols;
        }
