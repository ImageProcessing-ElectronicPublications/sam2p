...
bool GenBuffer::toBool(bool &dst)
{
    ...
    numtmp[0]|=32;
    numtmp[1]|=32;
    numtmp[2]|=32; /* poor man's uppercase */
    dst=true;
...
int GenBuffer::cmp(GenBuffer const& s2) const
{
    ...
    while (1)
    {
        ...
        if (0==(u1.len-=m))    next_sub(u1);
        else u1.beg+=m;
        if (0==(u2.len-=m)) s2.next_sub(u2);
        else u2.beg+=m;
    }
}
...
{
    slen_t got, sum=0;
    while (max>0 && 0<(got=vi_read(to_buf, max)))
    {
        to_buf+=got;
        sum+=got;
        max-=got;
...
SimBuffer::Static::Static(char const*cstr)
{
    beg=cstr;
    len=strlen(cstr);
}
...
void SimBuffer::B::vi_grow2(slendiff_t left, slendiff_t right, char **lbeg, char **rbeg)
{
    ...
    if (left<0)
    {
        if (len<=(slen_t)-left) len=0;    /* ! */
        else beg-=left;
        left=0;
    }
    if (right<0)
    {
        if (len<=(slen_t)-right)len=0;
        else len+=right;
        right=0;
    }
...
SimBuffer::B& SimBuffer::B::appendUnslash(const SimBuffer::Flat &other, int iniq)
{
    ...
    register char c;
    register char const*p=other.beg;
    if (iniq<=256)
    {
